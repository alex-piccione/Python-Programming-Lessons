...
class JustEatApiCaller:
    ...
    def get_restaurants(self, postcode):
        ...
        url = url_GET_restaurants.format(postcode) # format the strin template "injecting" the postcode
    ...
    def _get_menus(self, restaurant_id):
        ...
        url = url_GET_restaurant_menus.format(restaurant_id) # format the strin template "injecting" the restaurant ID
        headers = self._create_headers()
        response = requests.get(url, headers=headers) # make a request with GET verb (passing the headers) and store the response 
